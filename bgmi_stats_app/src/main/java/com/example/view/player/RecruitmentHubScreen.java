...
public class RecruitmentHubScreen {
    ...
    private VBox createRecruitmentCard(RecruitmentModel post) {
        ...
        topRow.getChildren().addAll(avatar, nameBox, spacer, timeBadge);
        ...
        applyBtn.setOnAction(e -> {
            ...
            alert.setHeaderText("Connect with " + post.getTeamName());
            alert.setContentText("Reach out to the roster manager externally using their Vault ID:\n\n"
                    + post.getAuthorId());

            // Apply GamerVault dark theme to the native JavaFX Alert
