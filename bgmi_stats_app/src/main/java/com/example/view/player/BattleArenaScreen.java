...
//         double userCoins = AuthController.currentUser != null ? AuthController.currentUser.getCoinBalance() : 0.0;
//         Text coins = new Text(String.format("Coins: %.0f 🪙", userCoins));
//         coins.setFill(Color.web("#0EA5E9")); // Upgraded to electric blue for visibility against charcoal
...
//         Text hostName = new Text("Host: Loading...");
//         hostName.setFill(Color.web(GamerVaultStyles.TEXT_MUTED));
//         hostName.setFont(Font.font("Arial", 10));

//         // Fetch the host's actual name in the background
//         new Thread(() -> {
//             com.example.model.UserModel hostUser = com.example.dao.ProfileDao.getUserProfileSync(battle.getHostUserId());
//             Platform.runLater(() -> {
//                 if (hostUser != null && hostUser.getIgn() != null) {
//                     hostName.setText("Host: " + hostUser.getIgn());
//                 } else {
//                     hostName.setText("Host: Unknown Target");
//                 }
//             });
//         }).start();
...
public class BattleArenaScreen {
    ...
    private HBox createControlsRow() {
        ...
        GamerVaultStyles.applyGradientButton(hostBtn, GamerVaultStyles.ACCENT_PURPLE,
                GamerVaultStyles.ACCENT_PURPLE_DARK, "white");
        attachRippleEffect(hostBtn, "#FFFFFF");
        hostBtn.setOnAction(e -> openHostBattleDialog());
    ...
    private VBox createFeaturedSection() {
        ...
        Text icon = new Text("⭐");
        icon.setFill(Color.web(GamerVaultStyles.ACCENT_PURPLE_LIGHT));
        GamerVaultAnimations.pulseGlow(icon, 2.5);
    ...
    private void filterLobbies() {
        ...
        if (playerLobbiesGrid.getChildren().isEmpty()) {
            ...
            Text emptyIcon = new Text(activeArenaTab.equals("ACTIVE") ? "⚔️" : "🔍");
            ...
            playerLobbiesGrid.getChildren().add(emptyState);
        }
    }
    ...
    private StackPane createPlayerLobbyCard(BattleModel battle) {
        ...
        if (battle.getStatus().equals(BattleFirebaseKeys.STATUS_OPEN)) {
            GamerVaultAnimations.pulseGlow(statusBadge, 2.0);
        }
        ...
        tLbl.setFill(Color.web(timeLeftMillis > 0 ? GamerVaultStyles.ACCENT_CYAN : "#EF4444"));
        tLbl.setFont(Font.font("Arial", FontWeight.BOLD, 12));
        if (timeLeftMillis > 0 && timeLeftMillis < 10 * 60 * 1000) {
            tLbl.setFill(Color.web("#F59E0B"));
            GamerVaultAnimations.pulseGlow(tLbl, 1.2);
        }
        ...
        double fillWidth = 220 * ((double) currentJoined / battle.getMaxParticipants());
        Rectangle fgTrack = new Rectangle(0, 4, Color.web(GamerVaultStyles.ACCENT_PURPLE_LIGHT));
        fgTrack.setArcWidth(4);
        fgTrack.setArcHeight(4);
        track.getChildren().addAll(bgTrack, fgTrack);

        javafx.animation.Timeline fillGrow = new javafx.animation.Timeline(
                new javafx.animation.KeyFrame(javafx.util.Duration.ZERO,
                        new javafx.animation.KeyValue(fgTrack.widthProperty(), 0)),
                new javafx.animation.KeyFrame(javafx.util.Duration.millis(600),
                        new javafx.animation.KeyValue(fgTrack.widthProperty(), fillWidth,
        ...
        if (alreadyJoined) {
            ...
            joinBtn.setStyle(joinBtn.getStyle() + "-fx-border-color: " + GamerVaultStyles.ACCENT_CYAN
                    + "; -fx-text-fill: white;");
            attachRippleEffect(joinBtn, "#FFFFFF");
    ...
    // --- UTILS ---
    /*
     * Material-style expanding ripple centered on the click point. Added via
     * addEventHandler (not setOnMousePressed) so it can never overwrite a press
     * handler another utility might already have on the same button, and it
     * never touches setOnAction - purely a visual response layered alongside
     * whatever click behavior already exists.
     */
